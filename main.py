...
def MYSQLconnectionCheck():
    ...
    if myConnection:
        ...
        cursor.execute("USE HMS")  # Switch to the 'HMS' database
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS C_DETAILS (
                CID VARCHAR(20),C_NAME VARCHAR(30),C_ADDRESS VARCHAR(30),C_AGE VARCHAR(30),
    ...
    if customer:
        if myConnection:
            ...
            createTable = """CREATE TABLE IF NOT EXISTS BOOKING_RECORD(
                                CID VARCHAR(20),CHECK_IN DATE ,CHECK_OUT DATE)
                            """
            cursor.execute(createTable)
    ...
    if customer:
        ...
        if myConnection:
            ...
            createTable = """CREATE TABLE IF NOT EXISTS ROOM_RENT(
                                CID VARCHAR(20),ROOM_CHOICE INT,NO_OF_DAYS INT,ROOMNO INT ,ROOMRENT INT)
                            """
            cursor.execute(createTable)
    ...
    if customer:
        ...
        if myConnection:
            ...
            createTable = """CREATE TABLE IF NOT EXISTS RESTAURENT(
                                CID VARCHAR(20),CUISINE VARCHAR(30),QUANTITY VARCHAR(30),BILL VARCHAR(30))
                            """
            cursor.execute(createTable)
    ...
    if customer:
        ...
        if myConnection:
            ...
            createTable = """CREATE TABLE IF NOT EXISTS TOTAL(
                                CID VARCHAR(20),C_NAME VARCHAR(30),ROOMRENT INT ,RESTAURENTBILL INT ,
    ...
    if customer:
        ...
        if myConnection:
            ...
            createTable = """CREATE TABLE IF NOT EXISTS GAMING(
                                CID VARCHAR(20),GAMES VARCHAR(30),HOURS VARCHAR(30),GAMING_BILL VARCHAR(30))
                            """
            cursor.execute(createTable)
            ...
            if game == 7:
                print("Thank you for visiting the Gaming section!")
                return
    ...
    if customer:
        ...
        if myConnection:
            ...
            createTable = """CREATE TABLE IF NOT EXISTS FASHION(
                                CID VARCHAR(20),DRESS VARCHAR(30),AMOUNT VARCHAR(30),BILL VARCHAR(30))
                            """
            cursor.execute(createTable)
...
def showDataFromDatabase(database_name):
    global userName
    global password
    global myConnection
    ...
    try:
        myConnection = mysql.connector.connect(
            host="localhost", user=userName, passwd=password, database=database_name,
        ...
        if myConnection:
            ...
            if tables:
                ...
                for table in tables:
                    ...
                    print("\nData in table '{}':".format(table_name))
                    if table_data:
                ...
                print("\nNo tables found in {} database.".format(database_name))
    ...
    if myConnection:
        ...
        if tables:
            ...
            for table in tables:
                ...
                print(table[table_name_key])
        else:
            print("\nNo tables found in HMS database.")
        cursor.close()
    else:
        print("\nERROR ESTABLISHING MYSQL CONNECTION !")
...
while True:
    ...
    if choice == 1:
        ...
        database_name = input("Enter the name of the database to show data from: ")
        showDataFromDatabase(database_name)
    elif choice == 12:
        break
    else:
        print("Sorry, May Be You Are Giving Me Wrong Input, Please Try Again !!!")
